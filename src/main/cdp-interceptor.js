...
class CDPInterceptor {
    ...
    this.pollingInterval = setInterval(async () => {
      try {
        ...
        for (const entry of entries) {
          if (
            entry.name.includes("batchAsyncGenerateVideoText") &&
            !this.processedSubmits.has(entry.name)
          ) {
            this.processedSubmits.add(entry.name);
            await this.fetchAndProcessResponse(entry.name, "submit");
          } else if (
            entry.name.includes("batchCheckAsyncVideoGenerationStatus") &&
            !this.processedPolls.has(entry.name)
          ) {
  ...
  async fetchAndProcessResponse(url, type) {
    try {
      ...
      const result = await this.cdpConnection.execute(
      ...
      if (result && result.body) {
        ...
        if (type === "submit") {
          this.logger?.info(
            `📥 Submit response: ${data.operations?.length || 0} operations`,
          );
          this.listeners.submit.forEach((cb) => cb(data));
        } else if (type === "poll") {
          this.logger?.info(
            `📥 Poll response: ${data.operations?.length || 0} operations`,
          );
