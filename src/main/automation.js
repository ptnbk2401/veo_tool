...
async function automateWithAPIQueue(profilePath, prompts, settings = {}) {
  ...
  logger.info(`Prepared ${promptsData.length} prompts: ${promptsData.map(p => `#${p.index}`).join(', ')}`);
  ...
  if (countAfter.count !== promptsData.length) {
    logger.error(`ERROR: Expected ${promptsData.length} prompts but found ${countAfter.count}!`);
    
    // Show which prompts are in DB
    const allPrompts = db.prepare('SELECT idx FROM prompts ORDER BY idx').all();
    logger.info(`Prompts in DB: ${allPrompts.map(p => `#${p.idx}`).join(', ')}`);
  }
  ...
  try {
    ...
    orchestrator = new SequentialOrchestrator(driver, cdpInterceptor, db, logger, {
    ...
    const statsInterval = setInterval(() => {
      ...
      logger.info(`[Stats] Prompts: queued=${stats.queued}, in_progress=${stats.in_progress}, done=${stats.done}, failed=${stats.failed}, timeout=${stats.timeout} | Ops: total=${stats.total_ops}, downloaded=${stats.downloaded}`);
...
async function automateWithTwoPhase(profilePath, prompts, settings = {}) {
  logger.warn("automateWithTwoPhase is deprecated, using automateWithAPIQueue instead");
 ...
 */
async function automateConcurrentGeneration(profiles, prompts, settings, maxConcurrent = 4) {
  logger.error("automateConcurrentGeneration not yet implemented with API-driven approach");
