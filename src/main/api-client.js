...
class VeoApiClient {
  ...
  static async extractBaseUrl(driver) {
    ...
    if (
      baseUrl.includes("labs.google") &&
      !baseUrl.includes("labs.google.com")
    ) {
  ...
  async makeRequest(method, path, body, retries = 3) {
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        this.logger?.warn(
          `API request attempt ${attempt}/${retries} failed: ${error.message}`,
        );
      ...
      const req = client.request(options, (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            reject(
              new Error(
                `Authentication failed: ${res.statusCode} ${res.statusMessage}`,
              ),
            );
          } else {
  ...
  async submitPrompt(promptText, settings = {}) {
    const body = {
      requestData: {
        ...
        promptInputs: [{ textInput: promptText }],
      },
    };

    this.logger?.info(
      `Submitting prompt via API: ${promptText.slice(0, 50)}...`,
    );
    ...
    this.logger?.info(
      `API response: ${response.operations?.length || 0} operations, credits: ${response.remainingCredits}`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = client.request(options, (res) => {
        if (res.statusCode === 200) {
          ...
          this.downloadVideo(redirectUrl, filePath, onProgress)
            .then(resolve)
            .catch(reject);
