    ...
    return new Promise((resolve) => {
      const checkInterval = setInterval(() => {
        ...
        if (inflight === 0 && queued === 0) {
          ...
          const pendingDownloads = this.db.db.prepare(`
  ...
  startFeeder() {
    this.feederInterval = setInterval(async () => {
      ...
      try {
        ...
        this.logger?.info(`[Feeder] Submitting prompt ${prompt.idx}: ${prompt.tail_slug}`);

        try {
          const response = await this.apiClient.submitPrompt(prompt.prompt_text, this.settings);

          if (response.operations && response.operations.length > 0) {
            // Success: mark in_progress and insert operations
            this.db.markPromptInProgress(prompt.id, response.operations);
            this.logger?.info(`[Feeder] Prompt ${prompt.idx} submitted successfully with ${response.operations.length} operations`);
          } else {
            // No operations returned
            this.db.markPromptFailed(prompt.id, 'No operations returned from API');
            this.logger?.error(`[Feeder] Prompt ${prompt.idx} failed: no operations`);
          }
        } catch (error) {
          // API error
          this.db.markPromptFailed(prompt.id, error.message);
          this.logger?.error(`[Feeder] Prompt ${prompt.idx} failed: ${error.message}`);
        }
      } catch (error) {
  ...
  startPoller() {
    const poll = async () => {
      ...
      try {
        ...
        this.logger?.info(`[Poller] Polling ${prompts.length} in-progress prompts`);
        ...
        for (const prompt of prompts) {
          try {
            ...
            if (response.operations && response.operations.length > 0) {
              ...
              if (newStatus === 'done') {
                this.logger?.info(`[Poller] Prompt ${prompt.idx} completed successfully`);
            ...
            this.logger?.error(`[Poller] Error polling prompt ${prompt.idx}: ${error.message}`);
          }
        }
      } catch (error) {
      ...
      if (this.running) {
        const jitter = Math.random() * this.POLLER_JITTER_MS * 2 - this.POLLER_JITTER_MS;
      ...
      try {
        ...
        try {
          ...
          if (error.message.includes('expired') || error.message.includes('403')) {
            this.logger?.info(`[Downloader-${workerId}] URL expired, will re-poll`);
