...
class SequentialOrchestrator {
    ...
    try {
      if (!data.operations || data.operations.length === 0) {
        ...
        if (this.currentSubmittingPrompt) {
          this.db.markPromptFailed(
            this.currentSubmittingPrompt.id,
      ...
      if (!prompt) {
        ...
        if (opName) {
          ...
          if (existingOp) {
            this.logger?.warn(
              `[CDP] Duplicate submit response for operation ${opName}, ignoring`,
            );
            return;
          }
        }
        ...
        const submittingPrompt = this.db
          .prepare(
            `
        ...
        if (submittingPrompt) {
          ...
          this.logger?.info(
            `[CDP] Matched submit response to prompt ${prompt.idx} by status`,
          );
        } else {
          this.logger?.warn(
            "[CDP] No matching prompt found for submit response",
          );
          return;
        }
      }
      ...
      if (existingOps.count > 0 && prompt.retry_count === 0) {
        this.logger?.warn(
          `[CDP] Prompt ${prompt.idx} already has operations, ignoring duplicate submit response`,
        );
        this.currentSubmittingPrompt = null;
        return;
      }
      ...
      this.logger?.info(
        `✅ [CDP] Prompt ${prompt.idx} submitted with ${data.operations.length} operations`,
      );
      ...
      this.logger?.error(
        `[CDP] Error handling submit response: ${error.message}`,
      );
    }
  }
    ...
    try {
      ...
      if (newStatus === "done") {
        const prompt = this.db
          .prepare("SELECT idx FROM prompts WHERE id = ?")
          .get(operation.prompt_id);
        ...
        const prompt = this.db
          .prepare(
            "SELECT idx, retry_count, max_retries FROM prompts WHERE id = ?",
          )
          .get(operation.prompt_id);
        const statusEmoji = newStatus === "failed" ? "❌" : "⏱️";
        this.logger?.warn(
          `${statusEmoji} [CDP] Prompt ${prompt?.idx} ${newStatus}`,
        );
        ...
        if (retryResult.canRetry) {
          ...
          this.logger?.error(
            `❌ [CDP] Prompt ${prompt?.idx} failed permanently: ${retryResult.reason}`,
          );
        }
      }
    } catch (error) {
      this.logger?.error(
        `[CDP] Error handling poll response: ${error.message}`,
      );
    }
  }
    ...
    return new Promise((resolve) => {
      const checkInterval = setInterval(() => {
        ...
        if (inflight === 0 && queued === 0 && submittingCount === 0) {
          ...
          const pendingDownloads = this.db
            .prepare(
              `
          ...
          if (pendingDownloads.count === 0) {
            this.logger?.info(
              `[Completion] All prompts processed. Stopping...`,
            );
  ...
  recoverStuckPrompts() {
    try {
      ...
      if (stuckPrompts.length > 0) {
        this.logger?.warn(
          `[Recovery] Found ${stuckPrompts.length} stuck prompts with operations`,
        );

        for (const prompt of stuckPrompts) {
          // Update status and check completion
          this.db
            .prepare(`UPDATE prompts SET status = 'in_progress' WHERE id = ?`)
            .run(prompt.id);
          const newStatus = this.db.checkAndUpdatePromptStatus(prompt.id);
          this.logger?.info(
            `[Recovery] Prompt ${prompt.idx}: submitting → ${newStatus}`,
          );
      ...
      this.logger?.error(
        `[Recovery] Error recovering stuck prompts: ${error.message}`,
      );
    }
  }
  ...
  startBrowserHealthCheck() {
    this.healthCheckInterval = setInterval(async () => {
      ...
      try {
        ...
        this.logger?.error(
          `[HealthCheck] Browser crashed or closed: ${error.message}`,
        );
        this.running = false;
      }
    }, 5000); // Check every 5s
    ...
    this.submitterInterval = setInterval(async () => {
      ...
      try {
        ...
        try {
          ...
          this.logger?.error(
            `[Submitter] Error getting stats: ${statsError.message}`,
          );
          return;
        }
        ...
        if (inflight >= this.MAX_IN_PROGRESS) {
          ...
          this.logger?.info(
            `[Submitter] Waiting for slot (${inflight}/${this.MAX_IN_PROGRESS})`,
          );
          return;
        }
        ...
        try {
          ...
          this.logger?.error(
            `[Submitter] Error peeking next prompt: ${promptError.message}`,
          );
        ...
        if (!prompt) {
          this.logger?.warn(
            `[Submitter] No prompt available despite queued=${queued}`,
          );
          return;
        }
        ...
        if (existingOps.count > 0 && prompt.retry_count === 0) {
          this.logger?.warn(
            `[Submitter] Prompt ${prompt.idx} already has operations, skipping and resetting to done`,
          );
        ...
        if (prompt.retry_count > 0) {
          this.logger?.info(
            `[Submitter] 🔄 Retry attempt ${prompt.retry_count} for prompt ${prompt.idx} (${existingOps.count} successful ops kept)`,
          );
        }

        this.logger?.info(
          `[Submitter] Got prompt ${prompt.idx}: ${prompt.prompt_text.slice(0, 50)}...`,
        );
        this.logger?.info(
          `[Submitter] Submitting prompt ${prompt.idx} (${inflight}/${this.MAX_IN_PROGRESS} in progress)`,
        );
        ...
        try {
          ...
          this.logger?.info(
            `[Submitter] Prompt ${prompt.idx} UI submission completed, waiting for CDP response...`,
          );
          ...
          while (
            this.currentSubmittingPrompt &&
            this.currentSubmittingPrompt.id === promptId &&
          ...
          if (
            ...
            this.logger?.warn(
              `[Submitter] No CDP response for prompt ${prompt.idx} after 10s, resetting to queued for retry`,
            );
            this.db.resetPromptToQueued(prompt.id);
            this.currentSubmittingPrompt = null;
          } else {
            this.logger?.info(
              `[Submitter] Prompt ${prompt.idx} processed by CDP successfully`,
            );
          }
        } catch (submitError) {
          this.logger?.error(
            `[Submitter] Failed to submit prompt ${prompt.idx}: ${submitError.message}`,
          );

          // Check if browser crashed
          if (
            submitError.message &&
            submitError.message.includes("invalid session")
          ) {
            this.logger?.error(
              `[Submitter] Browser session lost! Stopping automation.`,
            );
            this.running = false;
            return;
          }
  ...
  async submitPromptViaUI(promptText) {
    try {
      ...
      try {
        ...
        try {
          textarea = await this.driver.findElement(
            By.css('textarea[placeholder*="prompt" i]'),
          );
        } catch (e2) {
          try {
            textarea = await this.driver.findElement(
              By.css('textarea[aria-label*="prompt" i]'),
            );
          } catch (e3) {
  ...
  startPromptPoller(promptId) {
    ...
    const jitter =
      Math.random() * this.POLL_JITTER_MS * 2 - this.POLL_JITTER_MS;
  ...
  startDownloaders() {
    this.logger?.info(
      `[Downloader] Starting ${this.DOWNLOAD_CONCURRENCY} downloader workers...`,
    );
  ...
  async startDownloaderWorker(workerId) {
    ...
    while (this.running) {
      try {
        ...
        this.logger?.info(
          `[Downloader-${workerId}] Downloading prompt ${download.prompt_idx}, take ${download.take_index}`,
        );
        ...
        await new Promise((resolve, reject) => {
          ...
          const req = client.get(url, (res) => {
            if (res.statusCode === 200) {
              ...
              this.downloadWithRetry(redirectUrl, filePath, maxRetries)
                .then(resolve)
                .catch(reject);
            } else {
