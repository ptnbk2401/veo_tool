  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      try {
        ...
        await driver.executeScript("arguments[0].value = arguments[1];", element, text);
        // Trigger input event
      ...
      if (currentValue === text || currentValue.includes(text)) {
        ...
        throw new Error(`Text verification failed. Expected: ${text}, Got: ${currentValue}`);
      }
    } catch (error) {
      logger.warn(
...
async function generateVideoForPrompt(driver, prompt, profileId) {
  ...
  try {
      ...
      for (const button of buttons) {
        try {
          ...
          if (text.includes('Tạo') || text.includes('Generate') ||
            ariaLabel.includes('Tạo') || ariaLabel.includes('Generate')) {
            return true;
          }
        } catch (e) {
          continue;
    ...
    const allButtons = await driver.findElements(By.css('button:not([disabled])'));
    ...
    await driver.wait(async () => {
      try {
        const videoElements = await driver.findElements(By.css('video[src*="storage.googleapis.com"]'));
        if (videoElements.length > 0) {
          const videoSrc = await videoElements[0].getAttribute('src');
          return videoSrc && videoSrc.includes('storage.googleapis.com/ai-sandbox-videofx');
        }
        return false;
      } catch (error) {
        return false;
    ...
    if (await fs.pathExists(sourceFile)) {
      ...
      const urlFilePath = path.join("dist/videos", `${safeFilename}_profile-${profileId}_url.txt`);
      await fs.writeFile(urlFilePath, `Prompt: ${prompt}\n\nVideo URL:\n${videoSrc}`);
    }
...
function createVeoFilename(promptIndex, tailSlug, takeIndex, variant = 'veo3', durationSec = 8) {
...
function initializeTwoPhasePrompts(inputPrompts) {
    ...
    logger.info(`Initialized ${twoPhasePrompts.length} prompts for two-phase processing`);
...
async function twoPhaseSubmitPrompts(driver, textarea) {
    ...
    const canSubmitMore = () => twoPhaseInFlight.filter(j => !j.slotReleased).length < MAX_INFLIGHT_TWO_PHASE;
    ...
    const submitNextPrompt = async (jobId) => {
        ...
        logger.info(`Submitting prompt ${promptData.index}/${twoPhasePrompts.length}: ${promptData.tailSlug}`);

        const maxRetries = 3;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                // Wait for textarea to be ready and enabled
                logger.info(`Attempt ${attempt}/${maxRetries} to submit prompt ${promptData.index}`);
                ...
                await driver.executeScript(`
                    ...
                    textarea.focus();
                `, ta, promptData.promptText);
                
                await driver.sleep(300);
                
                // Verify prompt was entered correctly
                const currentValue = await ta.getAttribute('value');
                if (!currentValue || currentValue.trim() !== promptData.promptText.trim()) {
                    throw new Error(`Prompt verification failed. Expected length: ${promptData.promptText.length}, Got: ${currentValue?.length || 0}`);
                }
                
                logger.info(`Prompt ${promptData.index} entered and verified successfully`);
                ...
                }, 5000, 'Generate button not enabled');
                ...
                for (const button of buttons) {
                    try {
                        ...
                        if (text.includes("Tạo") || text.includes("Generate") || text.includes("Create")) {
                            await driver.executeScript("arguments[0].click();", button);
                            buttonClicked = true;
                            logger.info(`Generate button clicked for prompt ${promptData.index}`);
                            break;
                        }
                    } catch (e) {
                        continue;
                ...
                logger.info(`✅ Successfully submitted prompt ${promptData.index}/${twoPhasePrompts.length}`);
                return true;
                
            } catch (error) {
                logger.warn(`Attempt ${attempt}/${maxRetries} failed for prompt ${promptData.index}: ${error.message}`);
                ...
                if (attempt < maxRetries) {
                    ...
                    logger.error(`❌ Failed to submit prompt ${promptData.index} after ${maxRetries} attempts`);
    ...
    const pollInterval = setInterval(async () => {
        try {
            for (const job of twoPhaseInFlight) {
                ...
                if (timeSinceStart > 120000) { // 2 minutes timeout
                    job.done = true;
                    job.slotReleased = true;
                    logger.info(`Job ${job.id} completed (timeout)`);
                }
                
                if (!job.slotReleased && timeSinceStart > 10000) { // 10s release slot
                    job.slotReleased = true;
            ...
            if (queue.length > 0 || activeJobs > 0) {
                ...
                logger.info(`Phase A Status: submitted=${submitted}/${twoPhasePrompts.length}, queue=${queue.length}, active=${activeJobs}, slots_used=${slotsUsed}`);
            }
    ...
    // Wait for completion
    while (queue.length > 0 || twoPhaseInFlight.filter(j => !j.done).length > 0) {
...
async function extractTwoPhaseVideosFromItem(driver, item) {
    try {
        ...
        itemData.videoSrcs.forEach((src, index) => {
            ...
            const filename = createVeoFilename(matchedPrompt.index, matchedPrompt.tailSlug, takeIndex);
...
async function twoPhaseDownloadWorker(workerId, downloadQueue) {
    while (downloadQueue.length > 0) {
        ...
        if (success) {
            ...
            try {
                await fs.writeFile(urlFilePath, 
                ...
                logger.error(`Worker ${workerId} failed to save URL: ${urlError.message}`);
            }
        }
    }
}
...
async function twoPhaseDownloadVideoWithRetry(url, filePath) {
    for (let attempt = 1; attempt <= RETRY_MAX_TWO_PHASE; attempt++) {
        try {
            ...
            logger.warn(`Download attempt ${attempt}/${RETRY_MAX_TWO_PHASE} failed: ${error.message}`);
...
async function automateWithTwoPhase(profilePath, inputPrompts, settings = {}) {
    logger.info(`Starting two-phase automation with ${inputPrompts.length} prompts`);
