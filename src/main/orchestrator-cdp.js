...
class CDPOrchestrator {
  ...
  async handleSubmitResponse(data) {
    try {
      ...
      this.logger?.info(
        `✅ Prompt ${prompt.idx} submitted with ${data.operations.length} operations`,
      );
  ...
  startSubmitter() {
    this.submitterInterval = setInterval(async () => {
      ...
      try {
        ...
        if (inflight >= 5) {
          this.logger?.info(
            `[Submitter] Waiting... (${inflight}/5 in progress)`,
          );
          return;
        }
        ...
        this.logger?.info(
          `[Submitter] Submitting prompt ${prompt.idx}: ${prompt.tail_slug} (${inflight}/5 in progress)`,
        );
        ...
        try {
          ...
          this.logger?.info(
            `[Submitter] Prompt ${prompt.idx} submitted successfully`,
          );
        } catch (submitError) {
          this.logger?.error(
  ...
  async submitPromptViaUI(promptText) {
    try {
      ...
      if (!currentValue || currentValue.trim() !== promptText.trim()) {
        this.logger?.warn(
          `Prompt verification failed. Expected: ${promptText.length} chars, Got: ${currentValue?.length || 0} chars`,
        );
        // Try again
        await this.driver.executeScript(
          `
