...
function AccountManager() {
  ...
  const handleQuickLogin = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setIsAddingProfile(false);
        alert(
          "Profile created successfully! You can now use it for automation.",
        );
      }
    } catch (error) {
  ...
  const handleSaveEdit = async (profileId) => {
    ...
    try {
      ...
      await window.electronAPI.profiles.update(profileId, {
        name: editName.trim(),
      });
  ...
  const handleTestProfile = async (profileId) => {
    ...
    try {
      ...
      alert(
        `Test result: ${result.isLoggedIn ? "Logged in" : "Not logged in"}\nStatus: ${result.sessionStatus}`,
      );
    } catch (error) {
      ...
      <div className="card">
        ...
        {profiles.length === 0 ? (
          <div className="text-center text-gray-500">
            ...
            <p className="text-sm">
              Click "Detect Profiles" to automatically find Chrome profiles on
              your system.
            </p>
          </div>
        ) : (
              ...
              <tbody>
                {profiles.map((profile) => (
                  <tr key={profile.id}>
                    ...
                    <td>
                      <span
                        className={`status ${getStatusColor(profile.sessionStatus)}`}
                      >
        ...
        <div className="card">
          ...
          <div className="bg-blue-50 p-4 rounded-lg mb-4">
            ...
            <p className="text-sm text-blue-700 mb-3">
              We'll open a browser window for you to login to VEO3, then
              automatically save your profile.
            </p>
            <button
              className="btn btn-primary"
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <p className="text-sm text-gray-600 mb-3">
              If you already have a Chrome profile with VEO3 login, you can
              browse to select it.
            </p>
            ...
            <div className="form-group mb-3">
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setNewProfile((prev) => ({ ...prev, path: e.target.value }))
                  }
                />
                <button
                  className="btn btn-outline"
