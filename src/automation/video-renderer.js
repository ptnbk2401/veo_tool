...
class VideoRenderer {
      ...
      async (attempt) => {
        try {
          ...
          if (!flowUrl || flowUrl.trim() === "") {
            console.log(
              "🔄 No Flow URL provided, trying to use current page as project...",
            );
  ...
  async createNewVEO3Project(driver, options = {}) {
    try {
      ...
      try {
        ...
        console.log(
          "❌ Flow title not found, checking if page loaded anyway...",
        );
        console.log(
          `Title error details: ${titleError.name} - ${titleError.message}`,
        );
        ...
        try {
          ...
          console.log(
            "❌ Body element not found either, page may not be loading",
          );
          console.log(
            `Body error details: ${bodyError.name} - ${bodyError.message}`,
          );
  ...
  async inputPrompt(driver, prompt) {
    ...
    for (const selector of this.selectors.promptInput) {
      ...
      try {
        ...
        if (isDisplayed && isEnabled) {
          ...
          console.log(
            `❌ Element found but not usable (displayed: ${isDisplayed}, enabled: ${isEnabled})`,
          );
          promptElement = null;
        }
      } catch (e) {
    ...
    if (!promptElement) {
      ...
      console.log(
        "No prompt input found with specific selectors. Searching for any text inputs...",
      );

      const allTextInputs = await driver.findElements(
        By.css('textarea, input[type="text"], input:not([type])'),
      );
      ...
      for (let i = 0; i < Math.min(allTextInputs.length, 5); i++) {
        try {
          ...
          const placeholder =
            (await allTextInputs[i].getAttribute("placeholder")) || "";
          const id = (await allTextInputs[i].getAttribute("id")) || "";
          const className =
            (await allTextInputs[i].getAttribute("class")) || "";
          const isDisplayed = await allTextInputs[i].isDisplayed();

          console.log(
            `Input ${i + 1}: <${tagName}> id="${id}" class="${className}" placeholder="${placeholder}" visible=${isDisplayed}`,
          );
          ...
          console.log(
            `Could not inspect input ${i + 1}: ${debugError.message}`,
          );
        }
      }
      ...
      if (!promptElement) {
        ...
        console.log(
          `DEBUG: Page contains 'textarea': ${pageSource.includes("textarea")}`,
        );
        console.log(
          `DEBUG: Page contains 'PINHOLE_TEXT_AREA': ${pageSource.includes("PINHOLE_TEXT_AREA")}`,
        );
  ...
  async clickRender(driver) {
    ...
    for (const selector of this.selectors.renderButton) {
      ...
      try {
        ...
        if (isDisplayed && isEnabled) {
          ...
          console.log(
            `❌ Render button found but not usable (displayed: ${isDisplayed}, enabled: ${isEnabled})`,
          );
          renderButton = null;
        }
      } catch (e) {
        console.log(
          `❌ Render button selector failed: ${selector} - ${e.message}`,
        );
        renderButton = null;
      }
    }
    ...
    if (!renderButton) {
      ...
      console.log(
        "No render button found with specific selectors. Searching for any buttons...",
      );
      ...
      for (let i = 0; i < Math.min(allButtons.length, 10); i++) {
        try {
          ...
          console.log(
            `Button ${i + 1}: "${buttonText}" type="${buttonType}" class="${buttonClass}" visible=${isDisplayed}`,
          );
        } catch (debugError) {
          console.log(
            `Could not inspect button ${i + 1}: ${debugError.message}`,
          );
        }
      }
      ...
      for (const selector of veo3CompletionSelectors) {
        try {
          ...
          if (await element.isDisplayed()) {
            console.log(
              `✅ Render complete! Found download element: ${selector}`,
            );
            return true;
          }
        } catch (e) {
      ...
      for (const selector of videoIndicators) {
        try {
          ...
          if (await element.isDisplayed()) {
            ...
            if (src && src.length > 0) {
              console.log(
                `✅ Video has valid src: ${src.substring(0, 100)}...`,
              );
              ...
              try {
                ...
                if (duration && parseFloat(duration) > 0) {
                  ...
                  console.log(
                    `✅ Video element ready (duration not yet available)`,
                  );
                }
              } catch (e) {
      ...
      for (const selector of durationIndicators) {
        try {
          ...
          if (await element.isDisplayed()) {
            ...
            if (text.match(/^\d+:\d{2}$/)) {
              console.log(
                `✅ Video duration found: "${text}" - Video is ready!`,
              );
              return true;
            }
          }
  ...
  async getDetailedProgress(driver) {
    try {
        ...
        try {
          ...
          if (await element.isDisplayed()) {
            ...
            if (ariaValue && ariaMax) {
              progressData.percentage = Math.round(
                (parseInt(ariaValue) / parseInt(ariaMax)) * 100,
              );
            } else if (ariaValue) {
      ...
      try {
        ...
        const pageTitle = await driver.getTitle();
        console.log(
          `🌐 Current state - URL: ${currentUrl}, Title: ${pageTitle}`,
        );
      } catch (e) {
        // Ignore
      }
      ...
      const hasProgress =
        ...
        progressData.stage;
      console.log(
        `📊 Progress result: ${hasProgress ? JSON.stringify(progressData) : "No progress info found"}`,
      );
