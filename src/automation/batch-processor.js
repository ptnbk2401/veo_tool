...
class BatchProcessor {
  ...
  getNextPendingItem(jobId) {
    console.log(
      `🔍 BatchProcessor: Looking for next pending item in job ${jobId}`,
    );
    const job = this.activeJobs.get(jobId);
    if (!job) {
      console.log(`❌ BatchProcessor: Job ${jobId} not found in activeJobs`);
      console.log(
        `Available jobs: ${Array.from(this.activeJobs.keys()).join(", ")}`,
      );
      return null;
    }

    console.log(
      `📊 BatchProcessor: Job found, data length: ${job.data ? job.data.length : "undefined"}`,
    );
    if (job.data && job.data.length > 0) {
      console.log(`First item status: ${job.data[0].Status}`);
      const pendingCount = job.data.filter(
      ...
      if (pendingCount === 0) {
        ...
        if (errorItems.length > 0) {
          console.log(
            `🔄 BatchProcessor: No pending items, resetting ${errorItems.length} error items to pending for retry`,
          );
          ...
          console.log(
            `✅ BatchProcessor: Reset complete, now have ${job.data.filter((item) => item.Status === "pending").length} pending items`,
          );
        }
      }
    }
    ...
    console.log(
      `📋 BatchProcessor: Next item result: ${nextItem ? `ID ${nextItem.ID}` : "null"}`,
    );
    return nextItem;
  }
