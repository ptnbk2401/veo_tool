...
class AutomationManager {
  ...
  async startBatchProcessing(config) {
    try {
      ...
      const sessionTest =
        await this.profileManager.testProfileSession(profileId);
    ...
    return {
      ...
      async start() {
        ...
        try {
          ...
          console.log(
            "✅ AutomationManager: Browser session received, starting processing loop...",
          );
        ...
        while (this.isRunning) {
          try {
            ...
            if (!nextItem) {
              ...
              console.log(
                "✅ AutomationManager: No more items, completing job...",
              );
            ...
            console.log(
              `🎬 AutomationManager: Processing item ${nextItem.ID}: "${nextItem.Prompt.substring(0, 50)}..."`,
            );
            // Process the item
      ...
      async processItem(item) {
        ...
        try {
          ...
          console.log(
            `🎥 AutomationManager: Calling videoRenderer.renderVideo for item ${item.ID}`,
          );
      ...
      async completeJob() {
        ...
        // Complete job
        const completedJob =
          await automationManager.batchProcessor.completeJob(jobId);
